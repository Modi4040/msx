...
def ollama_status():
    ...
    try:
        resp = req.get(f"{OLLAMA_URL}/api/tags", timeout=4)
        resp.raise_for_status()
...
def ai_chat():
    """Proxy chat messages to Ollama running locally."""
    OLLAMA_URL  = os.environ.get("OLLAMA_URL",   "http://localhost:11434")
    ...
    try:
        resp = req.post(
            ...
            timeout=300,          # 5 min — first load can be slow
        )
        resp.raise_for_status()
        data = resp.json()
        return jsonify({"response": data.get("response", "")})
    except req.exceptions.ConnectionError:
        return jsonify({"error": "Ollama is not running. Open a terminal and run: ollama serve"}), 503
    except req.exceptions.Timeout:
        return jsonify({"error": "Ollama timed out. The model may still be loading — try again in 30 seconds, or run a smaller model: ollama pull llama3.2:1b"}), 504
    except req.exceptions.HTTPError as exc:
        code = exc.response.status_code
        text = exc.response.text[:300]
        if code == 404:
            return jsonify({"error": f"Model '{model}' not found. Run: ollama pull {model}"}), 404
        return jsonify({"error": f"Ollama error {code}: {text}"}), 502
    except Exception as exc:
        return jsonify({"error": str(exc)}), 500
